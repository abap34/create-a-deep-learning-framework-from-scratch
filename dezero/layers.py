...
class RNN(Layer):
    def __init__(self, hidden_size, in_size=None):
        super().__init__()
        self.x2h = Linear(hidden_size, in_size=None)
...
class LSTM(Layer):
    ...
    def forward(self, x):
        if self.h is None:   #最初
            f = F.sigmoid(self.x2f(x))  # forget gate
            i = F.sigmoid(self.x2i(x))  # input gate
            o = F.sigmoid(self.x2o(x))  # output gate
            u = F.tanh(self.x2u(x))     
        else:
        ...
        if self.c is None:
            c_new = (i * u)           # input gateに通す
        else:
            c_new = (f * self.c) + (i * u)  # forget gateとinput gateに通す
        
        h_new = o * F.tanh(c_new)      # output gateに通す
